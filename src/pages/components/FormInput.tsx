...
const FormInput: React.FC<InputProps & UseControllerOptions> = (props: InputProps & UseControllerOptions) => {
  const { field } = useController(props);
  const { type, setVal, isClear, isShowScore, isShowSendCode, pwChangeRight } = props;
  const { value } = field;
  ...
  return (
    <Wrap>
      <Input className={'password'} defaultValue={'' as any} {...field} {...props} type={type} autoComplete="off" />
      ...
      {type && (type === 'password' || type === 'psword') && (
        <PasswordChangeIcon pwChangeRight={pwChangeRight} isShowScore={isShowScore} onClick={switchType}>
          {psdType === 'password' || psdType === 'psword' ? (
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M4 8C4.7619 9.33333 7.42857 12 12 12C16.5714 12 19.2381 9.33333 20 8" stroke="#A7ACB8" />
...
const Input = styled.input<InputProps>`
  ...
  background: ${(props) => props.inputBackground || '#25272e'} !important;
  ...
  :-webkit-autofill {
    /* -webkit-box-shadow: 0 0 0px 1000px #2e323c inset; */
    color: #fff !important;
  }
...
const ClearIcon = styled.div<ClearProps>`
  ...
  position: absolute;
  right: ${(props) => {
    if (props.isPassword && !props.isShowScore) {
      return '40px';
    }
    ...
    if (props.isClear && props.isShowSendCode) {
      return '106px';
    }
